...
if (isset($_POST["email"]) && isset($_POST["login"]) && isset($_POST["password"]) && isset($_POST["passwordConf"]) && isset($_POST["lastname"]) && isset($_POST["firstname"])  && isset($_POST["genre"]) && isset($_POST["birth"])) {
...
if ($password == $_POST["passwordConf"] && $valide != false) {
    $stmt = MyPDO::getInstance()->prepare(<<<SQL
        INSERT INTO User (idUser, login, password, role, genre, email, lastname, firstname, birth, phone, sports, abilities) VALUES(:idUser, :login, :password, 0, :genre, :email, :lastname, :firstname, :birth, :phone, :sports, :abilities)
SQL);
